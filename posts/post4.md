...
# Writing the Harness
  ...
  feeds it through the FFmpeg decode pipeline, and exits. No GUI, no
  JNI, no Android framework.
...
# Building the Corpus
  ...
  ## Option 2: OSS-Fuzz corpus (more coverage, much slower)
...
# Running the Fuzzer
  ...
  ## Parallelizing with multiple instances
    ...
    target like this one, that might get you 50-200 exec/s. Spreading
    across cores multiplies that linearly. It's the easiest performance
...
# Conclusion
  ...
  The approach generalizes. The target here is libmigalleryffmpeg.so
  but the method works on any Android native library that processes
  external input and exposes a callable API. Pull the .so, trace the
  real app with Frida to find the pipeline, write a harness, cross-
  compile, run under QEMU. The steps are the same.
  ...
  that pull Android framework services need those services faked out.
  That's additional work, but it's scoped work, stub what the library
  asks for and nothing else.

  The other variable is speed. QEMU mode costs you a factor of 2-5x
  compared to native instrumented binaries. Persistent mode claws most
